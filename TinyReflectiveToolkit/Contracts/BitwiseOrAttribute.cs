...
namespace TinyReflectiveToolkit.Contracts
{
    ...
    /// application of the OR operator on the provided object and another object.
    /// </summary>
    [AttributeUsage(AttributeTargets.Method, Inherited = false, AllowMultiple = false)]
    public sealed class BitwiseOrAttribute : ExposeBinaryOperatorAttribute
    {
        /// <summary>
        /// Declares a proxy method to the operator, with the current object being on the specified side.
        /// </summary>
        /// <param name="side">The side on which the current instance will be provided to the OR operator (with the argument going to the other side).</param>
        public BitwiseOrAttribute(OpSide side)
            : base(side)
        {
        }
